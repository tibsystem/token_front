  ...
  smartSlimScroll.prototype = {
    init: function () {
      ...
      const handleWheel = function (e) {
        ...
        const shouldPreventScroll = scrollingUp && !atTop || scrollingDown && !atBottom || contentHeight > visibleHeight;
      ...
      try {
        ...
        var getBarHeight = function () {
          ...
          state.barHeight = Math.max(availableHeight / me.scrollHeight * availableHeight, state.minBarHeight);
        ...
        var showBar = function () {
          ...
          if (me.scrollHeight > wrapper.offsetHeight && (state.isOverPanel || state.isOverBar || state.isOverRail || state.isDragg || state.isScrolling)) {
            bar.style.opacity = o.opacity;
            rail.style.opacity = o.railOpacity;

            // If only hovering over panel (not interacting), hide after delay
            if (state.isOverPanel && !state.isOverBar && !state.isOverRail && !state.isDragg && !state.isScrolling) {
              clearTimeout(state.queueHide);
        ...
        var hideBar = function () {
          ...
          if (!o.alwaysVisible) {
            clearTimeout(state.queueHide);
            state.queueHide = setTimeout(function () {
              if (!state.isOverBar && !state.isOverRail && !state.isDragg && !state.isScrolling && !o.disableFadeOut) {
                bar.style.opacity = 0;
                rail.style.opacity = 0;
              }
            }, o.fadeOutSpeed);
        ...
        var onMouseDown = function (e) {
          ...
          var moveHandler = function (e) {
            ...
            currTop = Math.min(Math.max(currTop, offsetY), wrapper.offsetHeight - bar.offsetHeight - offsetY);
        ...
        wrapper.addEventListener('mouseleave', function (e) {
          ...
          if (!e.relatedTarget || !e.relatedTarget.closest('.' + o.barClass) && !e.relatedTarget.closest('.' + o.railClass)) {
        ...
        rail.addEventListener('mousedown', function (e) {
          ...
          let targetPos = Math.min(Math.max(clickPos - barHalfHeight, 0), availableHeight - bar.offsetHeight);
