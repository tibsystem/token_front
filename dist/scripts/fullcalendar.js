...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    height: undefined,
    // Let the calendar calculate its own height
    minHeight: '750px',
    // Set minimum height for desktop
    editable: true,
    droppable: true,
    // allow draggable events
    selectable: true,
    // allow selection for new events
    businessHours: {
    ...
    eventDidMount: function (info) {
      ...
      if (event.extendedProps.description || event.extendedProps.location) {
        ...
        if (!event.allDay && event.start) {
          ...
          timeEl.innerHTML = '<i class="fa fa-clock me-1"></i> ' + event.start.toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit'
          });
          if (event.end) {
  ...
  function populateCategoryList() {
    ...
    eventCategories.forEach((category, index) => {
      ...
      item.innerHTML = `
                ...
                ${!isDefaultCategory ? `<button type="button" class="btn btn-xs btn-outline-danger delete-category" data-id="${category.id}">
                        Delete
                    </button>` : ''}
            `;
  ...
  function filterEvents() {
    ...
    calendar.getEvents().forEach(event => {
      ...
      let eventCategory = event.extendedProps && event.extendedProps.category ? event.extendedProps.category : 'general';
      ...
      if (selectedCategories.length === 0 || selectedCategories.includes(eventCategory) || eventCategory === 'general' && includeGeneral) {
