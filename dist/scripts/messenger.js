document.addEventListener('DOMContentLoaded', () => {
  ...
  if (emojiButtons.length) {
    emojiButtons.forEach(emoji => {
      emoji.addEventListener('click', e => {
        ...
        const emojiType = emoji.classList.contains('emoji--like') ? '👍' : emoji.classList.contains('emoji--love') ? '❤️' : emoji.classList.contains('emoji--haha') ? '😂' : emoji.classList.contains('emoji--yay') ? '🎉' : emoji.classList.contains('emoji--wow') ? '😮' : emoji.classList.contains('emoji--sad') ? '😢' : emoji.classList.contains('emoji--angry') ? '😡' : '';
        if (emojiType) {
  ...
  function appendMessage(message, type, time) {
    ...
    const emojiType = message === '👍' ? 'like' : message === '❤️' ? 'love' : message === '😂' ? 'haha' : message === '🎉' ? 'yay' : message === '😮' ? 'wow' : message === '😢' ? 'sad' : message === '😡' ? 'angry' : null;
    let messageHtml;
    if (emojiType) {
      // Create the animated emoji without chat-message wrapper
      messageHtml = `
                <div class="emoji emoji--${emojiType}">
                    ${emojiType === 'like' ? `<div class="emoji__hand">
                            <div class="emoji__thumb"></div>
                        </div>` : emojiType === 'love' ? `<div class="emoji__heart"></div>` : emojiType === 'haha' ? `<div class="emoji__face">
                            <div class="emoji__eyes"></div>
                            <div class="emoji__mouth">
                                <div class="emoji__tongue"></div>
                            </div>
                        </div>` : emojiType === 'yay' ? `<div class="emoji__face">
                            <div class="emoji__eyebrows"></div>
                            <div class="emoji__mouth"></div>
                        </div>` : emojiType === 'wow' ? `<div class="emoji__face">
                            <div class="emoji__eyebrows"></div>
                            <div class="emoji__eyes"></div>
                            <div class="emoji__mouth"></div>
                        </div>` : emojiType === 'sad' ? `<div class="emoji__face">
                            <div class="emoji__eyebrows"></div>
                            <div class="emoji__eyes"></div>
                            <div class="emoji__mouth"></div>
                        </div>` : emojiType === 'angry' ? `<div class="emoji__face">
                            <div class="emoji__eyebrows"></div>
                            <div class="emoji__eyes"></div>
                            <div class="emoji__mouth"></div>
                        </div>` : ''}
  ...
  function appendFileMessage(file, type, time) {
    const messageDiv = document.createElement('div');
    messageDiv.className = `chat-segment chat-segment-${type}`;
    const fileIconClass = file.type === 'pdf' ? 'file-pdf text-danger' : file.type === 'doc' ? 'file-word text-primary' : file.type === 'xls' ? 'file-excel text-success' : file.type === 'ppt' ? 'file-powerpoint text-warning' : 'file text-muted';
    messageDiv.innerHTML = `
            <div class="chat-message">
  ...
  function populateInitialChat() {
    ...
    const messages = [{
      ...
      text: "Pretty good, thanks for asking! Just finished a big project.",
      type: 'sent',
      delay: 300
    }, {
      text: "That's great to hear! Is this the same one you mentioned last week?",
      type: 'get',
      delay: 600
    }, {
      text: "Yes, finally wrapped it up. The client was really happy with the results.",
      type: 'sent',
      delay: 900
    },
    //{ file: mockFiles[0], type: 'sent', isFile: true, delay: 1200 },
    {
      text: "Thanks for sharing the document! I'll take a look at it.",
      type: 'get',
      delay: 1500
    }, {
      text: "Let me know if you need any clarification.",
      type: 'sent',
      delay: 1800
    }, {
      image: mockImages[2],
      ...
      text: "That looks amazing! Is that from the project?",
      type: 'sent',
      delay: 2400
    }, {
      text: "Yes, it's the final design we went with 😊",
      type: 'get',
      delay: 2700
    }];
