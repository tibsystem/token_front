...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const defaults = {
    ...
    donutBackground: 'var(--bs-border-color)',
    // Light purple background instead of gray
    ...
    document.querySelectorAll(selector).forEach(element => {
      ...
      if (type === 'donut' || type === 'pie') {
        ...
        if (!options.fill || options.fill.length === 0) {
          options.fill = type === 'donut' ? ['var(--primary-500)', defaults.donutBackground] : ['var(--primary-500)', defaults.pieBackground];
        }
  ...
  try {
    ...
    document.querySelectorAll('[data-peity]').forEach(element => {
      ...
      if (element.classList.contains('peity-pie') || element.classList.contains('peity-donut') || element.classList.contains('peity-line') || element.classList.contains('peity-bar') || element.classList.contains('updating-chart') || element.classList.contains('bar-negative') || element.classList.contains('bar-transition')) {
        return;
      }
      ...
      if ((type === 'donut' || type === 'pie') && content.includes('/')) {
        ...
        if (!options.fill || !Array.isArray(options.fill) || options.fill.length < 2) {
          ...
          if (options.fill.length === 1) {
            ...
            options.fill = [type === 'donut' ? 'var(--primary-500)' : 'var(--success-500)', type === 'donut' ? defaults.donutBackground : defaults.pieBackground];
          }
        }
      }
