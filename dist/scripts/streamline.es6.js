...
function drawBarChart(svg, values, options) {
  ...
  values.forEach((val, i) => {
    if (val !== null && !isNaN(val)) {
      ...
      const color = val < 0 ? options.negBarColor : val === 0 ? options.zeroBarColor : options.barColor;
...
function drawDiscreteChart(svg, values, options) {
  ...
  values.forEach((val, i) => {
    if (val !== null) {
      ...
      const color = options.thresholdValue !== undefined && val < options.thresholdValue ? options.thresholdColor : options.lineColor;
      const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
...
function addInteraction(chartData) {
  ...
  const getPointColor = (index, value) => {
    if (options.type === 'pie' && pointCoordinates[0]?.sliceInfo[index]) {
      ...
      if (options.barColors) {
        return value < 0 ? options.barColors.negative : value === 0 ? options.barColors.zero : options.barColors.positive;
      }
      return value < 0 ? options.negBarColor : value === 0 ? options.zeroBarColor : options.barColor;
    } else if (options.type === 'tristate') {
      return value === -1 ? options.negBarColor : value === 0 ? options.zeroBarColor : options.posBarColor;
    } else if (options.type === 'discrete' && options.thresholdValue !== undefined && value < options.thresholdValue) {
  ...
  const findNearestPoint = (x, y) => {
    ...
    if (options.type === 'pie') {
      ...
      for (let i = 0; i < pointCoordinates.length; i++) {
        ...
        if (x >= barLeft - hitAreaExpansion && x <= barRight + hitAreaExpansion && y >= barTop - hitAreaExpansion && y <= barBottom + hitAreaExpansion) {
      ...
      for (let i = 0; i < pointCoordinates.length; i++) {
        ...
        if (Math.abs(x - point.x) <= hitAreaWidth / 2 && y >= point.lineTop && y <= point.lineTop + hitAreaHeight) {
  ...
  const highlightHoveredElement = index => {
    ...
    if (options.type === 'pie') {
      ...
      const largeArcFlag = sliceAngle > Math.PI ? 1 : 0;
      const pathData = [`M ${centerX} ${centerY}`, `L ${x1} ${y1}`, `A ${highlightRadius} ${highlightRadius} 0 ${largeArcFlag} 1 ${x2} ${y2}`, 'Z'].join(' ');
  ...
  const updateTooltipContent = index => {
    ...
    if (options.type === 'pie') {
      ...
      const {
        value,
        color
      } = pointCoordinates[0].sliceInfo[index];
