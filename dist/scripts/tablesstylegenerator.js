...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateTableStyle() {
    ...
    tableHeader.classList.remove('bg-primary', 'bg-secondary', 'bg-success', 'bg-danger', 'bg-warning', 'bg-info', 'bg-dark', 'bg-light', 'text-white');
    ...
    if (showRowClasses.checked) {
      tableRows.forEach((row, index) => {
        ...
        row.classList.remove('table-primary', 'table-secondary', 'table-success', 'table-danger', 'table-warning', 'table-info');
  ...
  function expandColumns() {
    ...
    hiddenCells.forEach(cell => {
      if (cell.textContent.includes('Address') || cell.textContent.includes('City') || cell.parentElement.cells[4] === cell || cell.parentElement.cells[5] === cell) {
        cell.classList.remove('d-none');
        cell.classList.add('d-md-table-cell');
      } else if (cell.textContent.includes('State') || cell.textContent.includes('Country') || cell.parentElement.cells[6] === cell || cell.parentElement.cells[7] === cell) {
        cell.classList.remove('d-none');
