...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const rowStates = {
    ...
    saved: new Set(),
    // Set of row IDs that should have the 'saved' class
  ...
  const clientTable = new SmartTables('clientTable', {
    ...
    data: {
      ...
      // Data format is JSON
      source: sampleData,
      // Use the sample data array as the data source
      idField: 'id',
      ...
      columns: [{
        ...
        options: [
        // Available options
        'Engineering', 'Sales', 'Executive', 'Marketing', 'Human Resources', 'Customer Support', 'Finance', 'Operations', 'Product', 'Research', 'Legal', 'IT']
      }, {
        data: 'joinDate',
        title: 'Join Date',
        type: 'date',
        ...
        render: data => data === true ? '<span class="badge bg-success">Yes</span>' // Visual indicator for active
        ...
        type: 'select',
        options: [
        // Options with separate value and label
        {
          value: 'Full-time',
          label: 'Full-time'
        }, {
        ...
        title: 'Actions',
        sortable: false,
        // Don't allow sorting by actions
        editable: false,
        // Not included in edit form
        render: (data, row) => `
    ...
    addRecord: true,
    // Enable the built-in Add Record button
    /**
     * Hooks Configuration
     * ------------------
        ...
        for (const field in columnTypes) {
          ...
          switch (type) {
            case 'number':
              ...
              processedData[field] = value !== undefined && value !== '' ? parseFloat(value) : 0;
              break;
            case 'boolean':
