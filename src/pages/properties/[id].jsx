...
export default function ImovelPage() {
      ...
      if (id) {
        try {
          ...
          if (response.total_value && response.total_tokens_original) {
            setValorUnitario(
              (
                Number(response.total_value) /
                Number(response.total_tokens_original)
              ).toFixed(2)
            );
          } else {
            setValorUnitario("");
          ...
          if (err?.response?.status === 401) {
            toast.error(
              "Sua sessão expirou ou você não tem permissão para acessar este imóvel. Faça login novamente se necessário."
            );
          } else {
      ...
      if (carouselElement && window.bootstrap) {
        setTimeout(() => {
          const existingInstance =
            window.bootstrap.Carousel.getInstance(carouselElement);
          if (!existingInstance) {
  ...
  const valorDisponivel =
    Number(property?.total_tokens || 0) * Number(valorUnitario || 0);
  const valorVendido =
    (Number(property?.total_tokens_original || 0) -
      Number(property?.total_tokens || 0)) *
  ...
  const isValid = () => {
    if (property?.status?.toLowerCase() === "pending") {
      toast.warning(
        "Este imóvel ainda está pendente de aprovação. A compra não está disponível no momento."
      );
      return false;
    }
    ...
    if (total < valorMinimo) {
      toast.warning(
        `Valor mínimo de investimento é R$ ${valorMinimo.toLocaleString(
          "pt-BR"
        )}`
      );
      return false;
    ...
    if (Number(amount) > Number(property?.total_tokens || 0)) {
      toast.warning(
        "A quantidade excede os tokens disponíveis para este imóvel."
      );
      return false;
    }
    return true;
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const data_compra = `${now.getFullYear()}-${pad(
        now.getMonth() + 1
  ...
  if (loading)
    return (
      <div className="p-5 text-muted fs-5 text-center">
        Carregando imóvel...
      </div>
    );
  if (error)
    return <div className="p-5 text-danger fs-5 text-center">{error}</div>;
  if (!property)
        ...
        <div className="card border-0 shadow-sm p-5 mb-5">
          <div className="row g-5">
            <div className="col-lg-7">
              <h1 className="h2 fw-bold mb-4 d-flex align-items-center gap-2">
                    ...
                    >
                      <div className="carousel-inner rounded-3 shadow-sm">
                        {(property.photos && property.photos.length > 0
                          ? property.photos
                          : [{ path: "/assets/img/default-property.jpg" }]
                        ).map((photo, idx) => (
                          <div
                          ...
                          >
                            <Image
                              src={
                                photo.path || "/assets/img/default-property.jpg"
                              }
                        ...
                        <div className="d-flex gap-2 overflow-auto pb-2">
                          {(property.photos || []).map((img, index) => (
                            ...
                            >
                              <Image
                                src={
                                  img.path || "/assets/img/default-property.jpg"
                                }
                                alt={`Imagem ${index + 1}`}
              ...
              <p className="mb-4 fs-5 text-primary fw-bold">
                Rentabilidade: {property.profitability}
              </p>
              ...
              <div className="p-3 bg-light rounded-3 mt-3">
                ...
                <p className="text-muted mb-2 small">
                  ...
                  <strong>Nível {property.level_warrant || 1}</strong>
                </p>
              ...
              <div className="row row-cols-2 g-3 text-muted fs-6 mb-4">
                <div>
                  ...
                  <br />
                  R${" "}
                  {Number(property.total_value || 0).toLocaleString("pt-BR", {
                    minimumFractionDigits: 2,
                  })}
                </div>
                <div>
                  <strong>Tokens Originais:</strong>
                  <br />
                  {property.total_tokens_original || 0}
                </div>
                <div>
                  <strong>Tokens Disponíveis:</strong>
                  <br />
                  {property.total_tokens || 0}
                </div>
                <div>
                  <strong>Valor Unitário:</strong>
                  <br />
                  R${" "}
                  {Number(valorUnitario || 0).toLocaleString("pt-BR", {
                    minimumFractionDigits: 2,
                  })}
                </div>
                    ...
                    width:
                      property.total_tokens_original > 0
                        ? `${
                            ((Number(property.total_tokens_original || 0) -
                              Number(property.total_tokens || 0)) /
                              Number(property.total_tokens_original || 1)) *
                            100
                          }%`
                        : "0%",
                  }}
              ...
              {property.tokenization_date && (
                <div className="d-flex align-items-center justify-content-between p-3 bg-light rounded-3">
                  ...
                  <span className="fw-semibold text-dark">
                    {new Date(property.tokenization_date).toLocaleDateString(
                      "pt-BR"
                    )}
                  </span>
                </div>
              ...
              <div className="card border-0 shadow-sm p-4">
                ...
                <div className="mb-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (val === "" || isNaN(val)) {
                        ...
                        let num = Math.max(1, Math.floor(Number(val)));
                        if (
                          property.total_tokens &&
                          num > property.total_tokens
                        ) {
                          num = property.total_tokens;
                        }
                        setAmount(num);
                  ...
                  <small className="text-muted">
                    Máximo disponível: {property.total_tokens || 0} tokens
                  </small>
                </div>
                <div className="mb-3">
                  ...
                  <div className="form-control form-control-lg bg-light">
                    R${" "}
                    {Number(valorUnitario).toLocaleString("pt-BR", {
                      minimumFractionDigits: 2,
                    })}
                  </div>
                </div>
