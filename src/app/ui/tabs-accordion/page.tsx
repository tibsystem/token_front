...
export default function UiTabsAccordions() {
	const initialState: (string | undefined)[] = Array(5).fill(undefined);
	...
	useEffect(() => {
  	...
  	const { signal } = controller;
		const fetchData = async (index: number) => {
			try {
				const response = await fetch(`/assets/data/ui/tabs-accordion-code-${index}.json`, { signal });
