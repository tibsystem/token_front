...
export default function PageOptionWithMixedMenu() {
    ...
    const controller = new AbortController();
  	const { signal } = controller;
		const fetchData = async (index: number) => {
			try {
				const response = await fetch(`/assets/data/page-option/with-mixed-menu-code-${index}.json`, { signal });
	...
	return (
        ...
        <li className="breadcrumb-item"><Link href="/page-option/with-mixed-menu">Home</Link></li>
        <li className="breadcrumb-item"><Link href="/page-option/with-mixed-menu">Page Options</Link></li>
