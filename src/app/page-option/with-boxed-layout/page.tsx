...
export default function PageOptionWithBoxedLayout() {
    ...
    const controller = new AbortController();
  	const { signal } = controller;
		const fetchData = async (index: number) => {
			try {
				const response = await fetch(`/assets/data/page-option/with-boxed-layout-code-${index}.json`, { signal });
	...
	return (
      ...
      <ol className="breadcrumb float-xl-end">
        <li className="breadcrumb-item"><Link href="/page-option/with-boxed-layout">Home</Link></li>
        <li className="breadcrumb-item"><Link href="/page-option/with-boxed-layout">Page Options</Link></li>
