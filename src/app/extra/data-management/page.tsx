...
export default function ExtraDataManagement() {
	const { updateSettings } = useAppSettings();
	
	const handleTableHeight = () => {
		const table = document.getElementById('table');
		if (table) {
			const targetHeight = window.innerHeight - table.getBoundingClientRect().top + 20;
			table.style.height = `${targetHeight}px`;
		}
	};

	const handleCheckbox = () => {
		document.querySelectorAll<HTMLInputElement>('[data-toggle="check-all"]').forEach((checkbox) => {
			checkbox.addEventListener('change', function () {
				const isChecked = (this as HTMLInputElement).checked;
				const table = this.closest('#table');
				if (table) {
					table.querySelectorAll<HTMLInputElement>('tbody input[type="checkbox"]').forEach((childCheckbox) => {
