...
export default function UiLanguageBarIcon() {
	...
	useEffect(() => {
  	...
  	const { signal } = controller;
		const fetchData = async (index: number) => {
			try {
				const response = await fetch(`/assets/data/ui/language-bar-icon-code-${index}.json`, { signal });
		...
		updateSettings({ appHeaderLanguageBar: true });
		
		return () => {
			controller.abort();
			updateSettings({ appHeaderLanguageBar: false });
		};
	...
	return (
		<>
			<ol className="breadcrumb float-xl-end">
				<li className="breadcrumb-item"><Link href="/ui/language-bar-icon">Home</Link></li>
				<li className="breadcrumb-item"><Link href="/ui/language-bar-icon">UI Elements</Link></li>
