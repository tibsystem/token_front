...
export default function PageOptionWithHoverSidebar() {
	...
	useEffect(() => {
  	...
  	const { signal } = controller;
		const fetchData = async (index: number) => {
			try {
				const response = await fetch(`/assets/data/page-option/with-hover-sidebar-code-${index}.json`, { signal });
	...
	return (
        ...
        <li className="breadcrumb-item"><Link href="/page-option/with-light-sidebar">Home</Link></li>
        <li className="breadcrumb-item"><Link href="/page-option/with-light-sidebar">Page Options</Link></li>
