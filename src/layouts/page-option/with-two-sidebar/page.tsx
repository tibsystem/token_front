...
export default function PageOptionWithTwoSidebar() {
	...
	useEffect(() => {
  	...
  	const { signal } = controller;
		const fetchData = async (index: number) => {
			try {
				const response = await fetch(`/assets/data/page-option/with-two-sidebar-code-${index}.json`, { signal });
	...
	return (
        ...
        <li className="breadcrumb-item"><Link href="/page-option/with-two-sidebar">Home</Link></li>
        <li className="breadcrumb-item"><Link href="/page-option/with-two-sidebar">Page Options</Link></li>
