...
export default function ExtraSettingsPage() {
  ...
  useEffect(() => {
    const sections = document.querySelectorAll<HTMLElement>('#bsSpyContent > div');
    const navLinks = document.querySelectorAll<HTMLAnchorElement>('#bsSpyTarget > a');

    const activateNavLink = (id: string | null) => {
      navLinks.forEach((link) => link.classList.remove('active'));

      if (id) {
        const target = document.querySelector<HTMLAnchorElement>(`#bsSpyTarget a[href="#${id}"]`);
        if (target) {
    ...
    const handleViewport = () => {
      ...
      if (currentActive !== activeSection) {
        ...
        activateNavLink(currentActive);
      }
    };

    window.addEventListener('scroll', handleViewport);
    handleViewport(); // Initial check on mount
